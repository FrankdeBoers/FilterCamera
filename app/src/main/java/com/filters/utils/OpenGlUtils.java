...
public class OpenGlUtils {
    ...
    public static int loadTexture(final Bitmap img, final int usedTexId, boolean recyled) {
        ...
        if (usedTexId == NO_TEXTURE) {
            ...
            Log.d("OpenGlUtils", "img ");
        } else {
            GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, usedTexId);
            GLUtils.texSubImage2D(GLES20.GL_TEXTURE_2D, 0, 0, 0, img);
            textures[0] = usedTexId;
            Log.d("OpenGlUtils", "usedTexId ");
        }
        if (recyled) {
            img.recycle();
        }
        Log.d("OpenGlUtils", "textures[0]:" + textures[0]);
        return textures[0];
    }
    ...
    public static String readShaderFromRawResource(final int resourceId) {
        final InputStream inputStream = FilterParams.context.getResources().openRawResource(
                resourceId);
    ...
    public static Bitmap drawToBitmapByFilter(Bitmap bitmap, GPUImageFilter filter,
        ...
        if (rotate) {
            ...
            FloatBuffer gLTextureBuffer = ByteBuffer.allocateDirect(com.filters.utils.TextureRotationUtil.TEXTURE_NO_ROTATION.length * 4)
                    .order(ByteOrder.nativeOrder())
                    .asFloatBuffer();
            gLTextureBuffer.put(com.filters.utils.TextureRotationUtil.getRotation(Rotation.ROTATION_90, true, false)).position(0);
